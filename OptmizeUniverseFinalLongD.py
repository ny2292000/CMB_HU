...
class HYPER:
    ...
    def getCMBxyz(self, results):
        self.calc_hyperharmnano2(self.karray)
        fcolor=self.df[:,4:].T.dot(results).squeeze()
    ...
    def getUniverseMap(self, results):
        n=100
        map3D= np.zeros([0,4])
        for i, r in enumerate(np.linspace(0.001,1,n)):
            self.recalcXYZ(r)
            map3D = np.concatenate([map3D,self.getCMBxyz(results)])
            print(i, map3D.shape)
        return map3D
