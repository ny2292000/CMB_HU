...
def newerr(x, SMICA_LR, newmap):
    ...
    err = np.sum(newnewmap*newnewmap)
    return err
...
if __name__ == "__main__":
    ...
    if Color.EVALUATE_DF_AT_POSITION in todo:
        ...
        nside3D = 64
        bandwidth = 2048
        filename = "./img1/SMICA_48_{}_{}_{}.png".format(kmax, nside3D, bandwidth)
        title = "SMICA_48_{}_{}_{}".format(kmax, nside3D, bandwidth)
        myHyper.plotNewMap(myHyper.SMICA_LR.squeeze(), err, filename=filename, title=title, plotme=True, save=True)
        #########################################################################
        nside3D = 1024
        bandwidth = 2048
        newmap_1024_0 = myHyper.change_resolution(newmap0, nside3D=1024, bandwidth=2048).squeeze()
        x01 = np.array([0.00072128, 0.47630094])
        x00 = minimize(newerr, x01, args=(myHyper.SMICA.squeeze(), newmap_1024_0.squeeze()),
                       method='nelder-mead', options={'xatol': 1e-8, 'disp': True})
        err = x00.fun
        xx0 = x00.x
        newmap_1024=xx0[1] * newmap_1024_0.squeeze() #+xx0[0]
        diffwmap_1024 = myHyper.SMICA.squeeze() - newmap_1024
        ...
        myHyper.plotNewMap(newmap_1024_0, err, filename=filename, title=title, plotme=True, save=True)
    ...
    if Color.OPTIMIZE_SMICA_BACKGROUND in todo:
        ...
        plt.show()
        freq = [centers[i + 1] - centers[i] for i in np.arange(4)]
