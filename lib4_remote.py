...
class HYPER:
    def __init__(self, nside3D, sigma_smica, planck_IQU_SMICA, karray, mypath,
        ...
        self.karray = karray
        self.mypath = mypath
        self.G_filename = os.path.join(self.mypath,
                                       "G_{}_{}_{}_{}.npy".format(self.nside3D, chg2ang(lambda_k), chg2ang(lambda_l),
                                                                  chg2ang(lambda_m)))
        self.P_filename = os.path.join(self.mypath,
                                       "P_{}_{}_{}_{}_".format(self.nside3D, chg2ang(lambda_k), chg2ang(lambda_l),
                                                               chg2ang(lambda_m)))
    ...
    def plot_aitoff(self, fcolors, kk, ll, mm, err, kmax, filename=None, title=None, plotme=False, save=False,
                    nosigma=True):
        # noinspection PyUnresolvedReferences
        plt.clf()
        if title is None:
    ...
    def plotNewMap(self, newmap, err, filename=None, title=None, plotme=False, save=False, nosigma=True):
        ...
        self.plot_aitoff(newmap, self.lambda_k, self.lambda_l,
                         self.lambda_m, self.kmax, err0, title=title, filename=filename, plotme=plotme, save=save,
                         nosigma=nosigma)
