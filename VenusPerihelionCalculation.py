...
def r_derivatives(theta, h, e, GM, x):
    ...
    d2r_dtheta2 = 2 * e ** 2 * h ** 2 * sigma ** 2 * np.sin(sigma * theta) ** 2 / (
                GM * (e * np.cos(sigma * theta) + 1) ** 3) + e * h ** 2 * sigma ** 2 * np.cos(sigma * theta) / (
                              GM * (e * np.cos(sigma * theta) + 1) ** 2)
...
def new_grav_accel(GM, r, r_dot, theta_dot):
    ...
    a_theoretical = -GM / r ** 2 / gamma_v / A / B
    #     a_theoretical = -GM /  r**2 / A / B
    return a_theoretical
...
df = pd.DataFrame(index=planet, columns=['delta_HU', "delta_GR"])
