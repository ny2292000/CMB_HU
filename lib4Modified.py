...
class HYPER:
    ...
    def plot_and_save_map(self, fcolor, radius, err, save_dir="./universeorthoview"):
        # Creates and saves two types of images for the universe map at a given radius.
        # :param fcolor: The color/frequency data for plotting.
        # :param radius: The current radius, used in the filename and title.
        # :param err: Error or any other metric for titling the image.
        # :param save_dir: Directory to save the images.
  
        if radius < 0.99:
            r_round = np.round(radius, 2)
        else:
            r_round = radius

        filename = f"{save_dir}/radius_{r_round}.png"
        filenameG = f"{save_dir}/globe_radius_{r_round}.png"
        title = f"Radius_{r_round}"
        
        # Plot and save the first image
        self.plotNewMap(fcolor, err, filename=filename, title=title, plotme=True, save=True)

        # Plot and save the second image
        stdd = fcolor.std()
        hp.orthview(fcolor.squeeze(), min=-3 * stdd, max=3 * stdd, half_sky=False, nest=False,
                    cmap='magma', title=title, unit=r'$\Delta$T (mK)')
        plt.savefig(filenameG)
        plt.show()


    def getUniverseMap(self, results, rr, save_images=False):
        map3D_list = []
        fcolors = {}
        
        for i in prange(len(rr)):
            self.recalcXYZ(rr[i])
            df, fcolor = self.getCMBxyz(i, results)
            map3D_list.append(df)
            fcolors[i] = fcolor

            if save_images:
                self.plot_and_save_map(fcolor, rr[i], err=0.0, save_dir="./universeorthoview")
        
        map3D = np.concatenate(map3D_list)
